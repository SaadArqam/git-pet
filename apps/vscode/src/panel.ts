...
import { PetDataService, PetData } from "./petData";

export class PetPanelProvider {
  public static currentPanel: PetPanelProvider | undefined;
  ...
  public static refresh(data: PetDataService): void {
    PetPanelProvider.currentPanel?.updateContent(data.getCached());
  }
  ...
  private updateContent(pet: PetData | null): void {
    this.panel.webview.html = this.buildHtml(pet);
  }

  private buildHtml(pet: PetData | null): string {
    if (!pet) {
      return `<!DOCTYPE html>
<html>
<head>
