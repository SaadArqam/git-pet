...
export default function LandingPage() {
  ...
  const [activeOverlay, setActiveOverlay] = useState<
    'about' | 'signin' | 'species' | 'leaderboard' | 'pets' | null
  >(null)
      ...
      const interactables: Obj[] = [
        ...
        { pos: new THREE.Vector3(0,0,-20), radius: 5, label: '[ E ]  Enter the shrine',
          onInteract: () => {
            ...
            setTimeout(() => { if (!mounted.current) return; openOverlay('signin') }, 900)
          }},
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', fontFamily: "'Syne', sans-serif", background: '#0d0f18' }}>
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 3, opacity: 0.028,
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 20, padding: '18px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(8,6,4,0.5)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(240,200,140,0.1)' }}>
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
          {isLoggedIn ? (
            <a href="/pet" style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: 2, textTransform: 'uppercase', color: '#ffd4a0', textDecoration: 'none', background: 'rgba(181,71,10,0.4)', border: '1px solid rgba(255,180,80,0.35)', padding: '9px 20px', backdropFilter: 'blur(8px)' }}>My Pet →</a>
          ) : (
            <button onClick={() => signIn('github', { callbackUrl: '/pet' })} style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 12, letterSpacing: 2, textTransform: 'uppercase', color: '#ffd4a0', background: 'rgba(26,20,14,0.7)', border: '1px solid rgba(240,200,140,0.25)', padding: '9px 20px', cursor: 'pointer', backdropFilter: 'blur(8px)' }}>Sign In →</button>
          )}
        </div>
      </nav>
        ...
        <div onClick={(e) => { if (e.target === e.currentTarget) openOverlay(null) }} style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(8,6,4,0.78)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'rgba(22,16,10,0.96)', border: '1px solid rgba(240,200,140,0.2)', padding: '48px 52px', maxWidth: 580, width: '90%', position: 'relative', maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            {activeOverlay === 'species' && <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 8 }}>
                {[{name:'Wolf',col:'#7a8070',role:'Aggro'},{name:'Saber',col:'#c5c0b8',role:'Tank'},{name:'Capy',col:'#8a6a3a',role:'Support'},{name:'Dragon',col:'#3d4a33',role:'Legend'},{name:'Axolotl',col:'#8a5a60',role:'Regen'}].map(s => (
                  <div key={s.name} style={{ border: `1px solid ${s.col}44`, padding: '16px 8px', textAlign: 'center', cursor: 'pointer', transition: 'background 0.2s' }}
                    ...
                    onClick={() => signIn('github', { callbackUrl: '/pet' })}>
              ...
              <button onClick={() => signIn('github', { callbackUrl: '/pet' })} style={{ ...overlayBtnStyle, marginTop: 24, width: '100%' }}>Sign In to Choose →</button>
            </>}
            ...
            {activeOverlay === 'pets' && <>
              ...
              <button onClick={() => signIn('github', { callbackUrl: '/pet' })} style={{ ...overlayBtnStyle, marginTop: 16, width: '100%' }}>Meet your pet →</button>
            </>}
          </div>
        </div>
      )}
